...
func providerConfigure(d *schema.ResourceData) (interface{}, error) {
	...
	childTokenLease, err := client.Auth().Token().Create(&api.TokenCreateRequest{
		...
		TTL:            fmt.Sprintf("%ds", d.Get("max_lease_ttl_sections").(int)),
		ExplicitMaxTTL: fmt.Sprintf("%ds", d.Get("max_lease_ttl_sections").(int)),
		Renewable:      &renewable,
	})
	if err != nil {
	...
	childToken := ""
	var ok bool
	if childToken, ok = childTokenLease.Data["client_token"].(string); !ok {
		return nil, fmt.Errorf("missing child_token in create token response")
	}
...
// obscureSecretInState should be applied as the StateFunc for any
// string attribute that contains secrets that are to be *written* to Vault.
//
// This means that the state will contain enough information for Terraform
// to detect a change but not to disclose the secret data.
//
// However, the stored value *is not salted or nonced*, so the value stored
// in state can still be used for brute-force attacks and it is possible to
// determine when two secrets have the same cleartext.
//
// This is *not* a suitable StateFunc for *Computed* attributes that contain
// secret data that is *read* from Vault; these *do* need to be persisted
// in the state as cleartext, so that they can be interpolated into other
// attributes within the configuration. The risk here is reduced by the
// fact that this provider intentionally requests short secret leases
// to limit exposure risk.
func obscureSecretInState(val interface{}) string {
	hashBytes := sha256.Sum256([]byte(val.(string)))
	return fmt.Sprintf("%064x", hashBytes[:])
}
